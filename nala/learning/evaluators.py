...
class MentionLevelEvaluator(Evaluator):
    ...
    def _find_offsets(dataset):
        ...
        return predicted_offsets, real_offsets
    ...
    def evaluate(self, dataset):
        # TODO write nice docstring
        predicted, real = self._find_offsets(dataset)
        ...
        if self.strictness is 'exact':
            ...
            precision = sum(1 for item in predicted if self._is_overlapping(item, real))/len(predicted)
            recall = sum(1 for item in real if self._is_overlapping(item, predicted))/len(real)
        elif self.strictness is 'half_overlapping':
            precision = sum(1 if item in real else 0.5 if self._is_overlapping(item, real) else 0
                            for item in predicted)/len(predicted)
            recall = sum(1 if item in predicted else 0.5 if self._is_overlapping(item, predicted) else 0
