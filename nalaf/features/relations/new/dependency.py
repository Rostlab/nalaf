...
class DependencyFeatureGenerator(EdgeFeatureGenerator):
    ...
    def add_n_grams(self, f_set, is_train, edge, path, dep_type, n_gram):
        ...
        add_groups('f_XX_bow_N_gram', path.strs_n_gram_token_only, lambda token: token.get_entity(edge.same_part).class_id if token.get_entity(part) else t.features['lemma'])
