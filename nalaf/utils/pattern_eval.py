...
import re
import time
from nalaf.preprocessing.definers import ExclusiveNLDefiner, InclusiveNLDefiner
...
def pattern_stats(dataset):
    ...
    :type nalaf.structures.Dataset: dataset to perform pattern evaluation on (must include annotations)
    ...
    with open('nalaf/data/connecting_words.json', 'r') as f:
        regexs = json.load(f)
    ...
    # read in nl_patterns
    with open('nalaf/data/nl_patterns.json', 'r') as f:
        regexs = json.load(f)
